...
import type { ItineraryTemplate } from "../../../store/api";

type DayEntry = { dayNumber: number; title: string; description: string };

export default function AdminItineraryTemplatesPage() {
  const { data, isLoading } = useGetItineraryTemplatesQuery();
  const [createTemplate, { isLoading: isCreating }] = useCreateItineraryTemplateMutation();
  const [updateTemplate, { isLoading: isUpdating }] = useUpdateItineraryTemplateMutation();
  const [deleteTemplate, { isLoading: isDeleting }] = useDeleteItineraryTemplateMutation();
  ...
  const handleEdit = (t: ItineraryTemplate) => {
    setEditingId(t._id);
    setName(t.name);
    setDays(
      (t.days ?? []).map((d) => ({
    ...
    try {
      if (editingId) {
        await updateTemplate({
          id: editingId,
          body: { name: name.trim(), days: validDays },
        }).unwrap();
      } else {
        await createTemplate({ name: name.trim(), days: validDays }).unwrap();
      }
      resetForm();
    } catch (err: unknown) {
      setError(
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Delete this itinerary template?")) return;
    setError(null);
    try {
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Create reusable itineraries. When creating a package, select one to attach.
        </p>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g. 6D Sikkim Darjeeling"
                ...
                {days.map((day, i) => (
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <span className="text-xs font-medium text-gray-500">Day {i + 1}</span>
                      <button
                        type="button"
                        onClick={() => removeDay(i)}
                    ...
                    <input
                      ...
                      placeholder="Title"
                      className="w-full rounded border border-gray-200 dark:border-gray-700 px-2 py-1 text-sm"
                    />
                    <textarea
                      ...
                      rows={2}
                      className="w-full rounded border border-gray-200 dark:border-gray-700 px-2 py-1 text-sm resize-none"
                    />
                  </div>
                ))}
              </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-700">
                  ...
                  <th className="pb-3 pr-3 text-left text-[11px] font-bold uppercase text-gray-400">Days</th>
                ...
                {isLoading ? (
                  ...
                  list.map((t) => (
                    <tr key={t._id} className="hover:bg-gray-50/60 dark:hover:bg-gray-800/60">
                      ...
                      <td className="py-3 pr-3 text-gray-600 dark:text-gray-400">{t.days?.length ?? 0} days</td>
