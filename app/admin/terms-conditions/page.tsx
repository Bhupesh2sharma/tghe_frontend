...
import type { TermsConditionTemplate } from "../../../store/api";

export default function AdminTermsConditionsPage() {
  const { data, isLoading } = useGetTermsConditionTemplatesQuery();
  const [createTemplate, { isLoading: isCreating }] = useCreateTermsConditionTemplateMutation();
  const [updateTemplate, { isLoading: isUpdating }] = useUpdateTermsConditionTemplateMutation();
  const [deleteTemplate, { isLoading: isDeleting }] = useDeleteTermsConditionTemplateMutation();
  ...
  const handleEdit = (t: TermsConditionTemplate) => {
    setEditingId(t._id);
    setName(t.name);
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Create reusable terms & conditions. Select one when creating a package.
        </p>
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <div className="rounded-2xl bg-white dark:bg-gray-800 p-6 shadow-sm ring-1 ring-gray-100 dark:ring-gray-700 lg:col-span-1">
          <h2 className="text-sm font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-4">
            {editingId ? "Edit terms" : "Create terms"}
          </h2>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {isLoading ? (
                  ...
                  <tr><td colSpan={2} className="py-6 text-center text-gray-500">No terms yet.</td></tr>
                ) : (
                  list.map((t) => (
